...
object WikipediaRanking {
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap {
      article => {
        val filteredLangs = langs.filter(lang => article.mentionsLanguage(lang))
        filteredLangs.map(lang => (lang, article))
      }
    }.groupByKey()
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues(_.count(_ => true)).sortByKey(false).collect().toList
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap {
      article => {
        val filteredLang = langs.filter(lang => article.mentionsLanguage(lang))
        filteredLang.map((_, 1))
      }
